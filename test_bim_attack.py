...
import math
def parse_args():
    parser = argparse.ArgumentParser(description='Test model with optional BIM attack')
    ...
    parser.add_argument('--attack', action='store_true', help='Perform BIM attack')
    parser.add_argument('--epsilons', nargs='+', type=float, default=[2, 4, 6, 8, 10], help='Epsilon values for BIM attack')
...
def bim_attack(model, image, mask,criterion, epsilon, alpha , device):
    img, msk = image.to(device).float(), mask.to(device).float()
    img.requires_grad=True 
    num_iters = math.ceil(min(epsilon+4,1.25*epsilon))
    for _ in range(num_iters):
        ...
        delta = alpha * torch.sign(img.grad.data) 
        img = torch.tensor(img.data + delta, requires_grad=True)
        clipped_delta = torch.clamp(img.data-ori_img.data, -epsilon*0.1/255., epsilon*0.1/255.)
        img = torch.tensor(ori_img.data+clipped_delta, requires_grad=True)
    return torch.clamp(img, 0., 1.)

def test_one_epoch_attack_bim(test_loader, model, criterion, threshold, epsilons, device, alpha = 2):
    model.eval()
    for eps in epsilons:
        print(f"Testing with BIM attack, epsilon: {eps}")
        preds = []
        gts = []
        loss_list = []
        for img, msk in test_loader:
            perturbed_data = bim_attack(model, img, msk, eps, criterion, device)
...
def main():
    ...
    for path in args.model_paths:
        ...
        for scenario_idx, dataloader in enumerate(test_dataloaders):
            ...
            if args.attack:
                test_one_epoch_attack_bim(dataloader, model, criterion, args.threshold, args.epsilons, device)
            else:
